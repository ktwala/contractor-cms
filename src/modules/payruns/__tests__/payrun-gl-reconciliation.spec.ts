import { ForbiddenException } from '@nestjs/common';
import { PayrunGLReconciliationStatus } from '@prisma/client';
import {
  PayrunGLReconciliationService,
  PAYRUN_GL_OVERRIDE_PERMISSION,
} from '../payrun-gl-reconciliation.service';
import { PrismaService } from '../../../core/database/prisma.service';
import { AuditService } from '../../../core/audit/audit.service';
import { PayrunGovernancePolicyResolutionService } from '../payrun-governance-policy-resolution.service';

describe('PayrunGLReconciliationService.assertAllowsPeriodCloseGl', () => {
  const make = (row: {
    status: PayrunGLReconciliationStatus;
    reviewedAt: Date | null;
    reviewRequired: boolean;
  } | null) => {
    const prisma = {
      payRun: {
        findUnique: jest.fn().mockResolvedValue({
          payGroupId: 'pg1',
          payGroup: { legalEntityId: 'le1' },
        }),
      },
      payrunGLReconciliation: {
        findUnique: jest.fn().mockResolvedValue(
          row
            ? {
                id: 'g1',
                payrunId: 'pr1',
                status: row.status,
                reviewedAt: row.reviewedAt,
                reviewRequired: row.reviewRequired,
                blockedReasonsJson: [],
                softWarningsJson: [],
              }
            : null,
        ),
      },
    };
    const audit = { log: jest.fn().mockResolvedValue(undefined) };
    const govPolicy = {
      resolvePayrunContext: jest.fn().mockResolvedValue({ legalEntityId: 'le1', payGroupId: 'pg1' }),
      resolveJustificationMinForContext: jest.fn().mockResolvedValue({
        value: 20,
        source: 'default',
        policy_id: null,
        policy_key: 'override.justification_min_length',
      }),
    };
    const svc = new PayrunGLReconciliationService(
      prisma as unknown as PrismaService,
      audit as unknown as AuditService,
      govPolicy as unknown as PayrunGovernancePolicyResolutionService,
    );
    return { svc, audit };
  };

  it('allows MATCH without override', async () => {
    const { svc, audit } = make({
      status: PayrunGLReconciliationStatus.MATCH,
      reviewedAt: null,
      reviewRequired: false,
    });
    await expect(
      svc.assertAllowsPeriodCloseGl('pr1', { sub: 'u1', permissions: [] }, {}, 'payroll.period_close'),
    ).resolves.toBeUndefined();
    expect(audit.log).not.toHaveBeenCalled();
  });

  it('allows VARIANCE after review', async () => {
    const { svc } = make({
      status: PayrunGLReconciliationStatus.VARIANCE,
      reviewedAt: new Date(),
      reviewRequired: false,
    });
    await expect(
      svc.assertAllowsPeriodCloseGl('pr1', { sub: 'u1', permissions: [] }, {}, 'payroll.period_close'),
    ).resolves.toBeUndefined();
  });

  it('allows soft-only VARIANCE when review is not required', async () => {
    const { svc } = make({
      status: PayrunGLReconciliationStatus.VARIANCE,
      reviewedAt: null,
      reviewRequired: false,
    });
    await expect(
      svc.assertAllowsPeriodCloseGl('pr1', { sub: 'u1', permissions: [] }, {}, 'payroll.period_close'),
    ).resolves.toBeUndefined();
  });

  it('blocks when GL recon row missing', async () => {
    const { svc } = make(null);
    await expect(
      svc.assertAllowsPeriodCloseGl('pr1', { sub: 'u1', permissions: [] }, {}, 'payroll.period_close'),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('blocks BLOCKED without override', async () => {
    const { svc } = make({
      status: PayrunGLReconciliationStatus.BLOCKED,
      reviewedAt: null,
      reviewRequired: true,
    });
    await expect(
      svc.assertAllowsPeriodCloseGl('pr1', { sub: 'u1', permissions: ['payroll:periods:close'] }, {}, 'payroll.period_close'),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('allows audited GL override', async () => {
    const { svc, audit } = make({
      status: PayrunGLReconciliationStatus.BLOCKED,
      reviewedAt: null,
      reviewRequired: true,
    });
    await expect(
      svc.assertAllowsPeriodCloseGl(
        'pr1',
        { sub: 'u1', permissions: [PAYRUN_GL_OVERRIDE_PERMISSION] },
        { overrideHeader: 'approved', justification: 'x'.repeat(20) },
        'payroll.period_close',
      ),
    ).resolves.toBeUndefined();
    expect(audit.log).toHaveBeenCalledWith(
      expect.objectContaining({
        action: 'PAYRUN_GL_RECON_OVERRIDE',
        entityId: 'pr1',
      }),
    );
  });
});

...
import { PayrollSupplementalImportService } from './payroll-supplemental-import.service';

@Controller('data-imports/payroll-supplemental')
@UseGuards(AuthGuard('jwt'), PermissionsGuard)
export class PayrollSupplementalImportController {
  constructor(private readonly service: PayrollSupplementalImportService) {}

  @Post('precheck')
  @AnyPermissions('data_import:write', 'data_import:approve')
  @UseInterceptors(FileInterceptor('file'))
  async precheck(@UploadedFile() file: Express.Multer.File) {
    if (!file?.buffer) {
  ...
  async precheckExportErrors(
    ...
    const filename = `supplemental-precheck-validation.${fmt}`;
  ...
  async getPreview(
    ...
    return this.service.getPreview(importJobId, userId);
  }
  ...
  async execute(
    ...
    if (!body?.confirm) {
      throw new BadRequestException('Confirmation required: { "confirm": true }');
    }

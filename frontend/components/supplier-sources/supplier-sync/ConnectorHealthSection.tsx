...
export function ConnectorHealthSection({ health }: Props) {
  return (
    <section className="card border-l-4 border-l-indigo-500" data-testid="connector-health-section">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Supplier Portal reachable ≠ Operational Trust granted in EWP
          </p>
        </div>
        <span
      ...
      {health.isStale && (
        <p
          ...
          role="status"
        >
          Sync data is stale (threshold: {health.staleThresholdHours}h). Do not treat connector as
          healthy.
        </p>
      )}
    </section>

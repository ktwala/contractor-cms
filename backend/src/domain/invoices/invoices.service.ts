...
export class InvoicesService {
  ...
  async generateFromTimesheets(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const lockedTimesheets = await tx.$queryRaw<LockedTimesheet[]>(
        Prisma.sql`SELECT id, "invoiceId", status FROM "Timesheet" WHERE id IN (${Prisma.join(dto.timesheetIds)}) ORDER BY id FOR UPDATE`
      );
  ...
  async submit(accessContext: AccessContext, id: string): Promise<InvoiceResponseDto> {
    const where: any = { id };
    if (!accessContext.isGlobalAccess) {
      where.organizationId = accessContext.targetOrganizationId;
    }

...
import type { HcmSponsorLookupProvider } from './hcm-sponsor-lookup.interface';
 ...
 */
@Injectable()
export class HcmSponsorLookupService implements HcmSponsorLookupProvider {
  ...
  async sponsorExists(_organizationId: string, _employeeId: string): Promise<boolean> {
   ...
   */
  async assertSponsorReferencesAllowed(
    ...
    const check = async (label: 'sponsorEmployeeId' | 'sponsorDelegateEmployeeId', value: string | null) => {
      ...
      if (!(await this.sponsorExists(organizationId, value))) {
        throw new BadRequestException(
    ...
    await check('sponsorEmployeeId', sponsor.sponsorEmployeeId);
    await check('sponsorDelegateEmployeeId', sponsor.sponsorDelegateEmployeeId);
  }
}
